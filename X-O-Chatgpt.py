...
class TIC_TAC_TOE:
    ...
    def on_button_click(self, row, col):
        ...
        if not button["text"]:
            button.config(text=self.current_player, background="red" if self.current_player == self.player_symbol else "blue")
    ...
    def toggle_board_color(self):
        # No need for this method if you're setting background colors in on_button_click
        pass
    ...
    def computer_move(self):
        ...
        if available_moves:
            ...
            self.buttons[row][col].config(text=self.computer_symbol, background="blue")
    ...
    def restart_game(self):
        for i in range(3):
            for j in range(3):
                self.buttons[i][j].config(text="", background="SystemButtonFace")
